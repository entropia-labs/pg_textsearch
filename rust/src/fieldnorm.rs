 ...
 *
 * fieldnorm.rs - Document length quantization for BM25 scoring
 *
 * Uses Lucene's SmallFloat encoding scheme which maps document lengths
 * to a single byte (256 buckets). This reduces storage while
 * maintaining good BM25 ranking quality because:
...
#[rustfmt::skip]
static FIELDNORM_DECODE_TABLE: [u32; 256] = [
    // 0-15: exact values
...
pub fn encode_fieldnorm(length: u32) -> u8 {
    let mut lo: usize = 0;
...
pub fn decode_fieldnorm(norm_id: u8) -> u32 {
    FIELDNORM_DECODE_TABLE[norm_id as usize]
}

// --- FFI exports ---

#[no_mangle]
pub extern "C" fn tp_rust_encode_fieldnorm(length: u32) -> u8 {
    encode_fieldnorm(length)
}

#[no_mangle]
pub extern "C" fn tp_rust_decode_fieldnorm(norm_id: u8) -> u32 {
    decode_fieldnorm(norm_id)
}
...
mod tests {
    ...
    #[test]
    fn test_exact_values_0_to_39() {
        for i in 0..=39u32 {
            let encoded = encode_fieldnorm(i);
            let decoded = decode_fieldnorm(encoded);
            assert_eq!(
                decoded, i,
                "length {} encoded to {} decoded to {}",
                i, encoded, decoded
            );
        }
    }
    ...
    fn test_roundtrip_monotonic() {
        ...
        let mut prev_encoded = 0u8;
        for length in 0..=1_000_000u32 {
            let encoded = encode_fieldnorm(length);
            assert!(
                encoded >= prev_encoded,
                "encoding not monotonic at length {}",
                length
            );
    ...
    #[test]
    fn test_decode_table_monotonic() {
        for i in 1..256 {
            assert!(
                FIELDNORM_DECODE_TABLE[i] >= FIELDNORM_DECODE_TABLE[i - 1],
                "decode table not monotonic at index {}",
                i
            );
        }
    }
    ...
    fn test_encode_decode_roundtrip() {
        // For exact values (0-39), roundtrip should be exact
        for length in 0..=39u32 {
            assert_eq!(decode_fieldnorm(encode_fieldnorm(length)), length);
        }

        // For larger values, decoded should be <= original
        for length in [40, 100, 500, 1000, 10000, 100_000, 1_000_000u32] {
            let encoded = encode_fieldnorm(length);
            let decoded = decode_fieldnorm(encoded);
            assert!(
                decoded <= length,
                "decoded {} > original {} for norm {}",
                decoded,
                length,
                encoded
            );
    ...
    fn test_boundary_values() {
        assert_eq!(encode_fieldnorm(0), 0);
        assert_eq!(decode_fieldnorm(0), 0);
        assert_eq!(encode_fieldnorm(u32::MAX), 255);
        assert_eq!(decode_fieldnorm(255), 2013265944);
    }
}
